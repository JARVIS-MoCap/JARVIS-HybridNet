...
class ReprojectionTool(nn.Module):
    def __init__(self, root_dir = None, calib_paths = None):
        ...
        if calib_paths != None:
            ...
            self.cameraMatrices = torch.zeros((self.num_cameras, 4,3), device = torch.device('cuda'))
            self.intrinsicMatrices = torch.zeros((self.num_cameras, 3,3), device = torch.device('cuda'))
            self.distortionCoefficients = torch.zeros((self.num_cameras, 1,5), device = torch.device('cuda'))
            ...
            self.cameraMatrices = torch.tensor(0, device = torch.device('cuda'))
            self.intrinsicMatrices = torch.tensor(0, device = torch.device('cuda'))
            self.distortionCoefficients = torch.tensor(0, device = torch.device('cuda'))


    def reprojectPoint(self,point3D):
        ones = torch.ones([point3D.shape[0],1], device=torch.device('cuda'))
