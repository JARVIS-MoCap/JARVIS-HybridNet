...
def plot_error_histogram(path, cutoff = -1):
    ...
    f, (ax_hist, ax_box) = plt.subplots(2, sharex=True, gridspec_kw= {"height_ratios": (1, 0.2)}, figsize=(6.92913,6.92913 / 1.618))
    distances_l = {}
...
def plot_error_per_joint(path):
    ...
    for i in range(0,len(joints)):
        plt.bar(joints[i], joints_means[i], width = barWidth, color = cmap(i*(1/number_joints)))

    plt.xticks([r-1 + barWidth for r in range(len(joints))], joint_labels, rotation=90)
    plt.show()
    ...
    for item in tqdm(range(len(dataset.image_ids))):
        ...
        if len(camsToUse) >= 2:
            ...
            for idx,reproPoint in enumerate(reproPoints):
                ...
                reproPoints[idx][0] = min(max(reproPoint[0], bbox_hw), img_size[1]-1-bbox_hw)
                reproPoints[idx][1] = min(max(reproPoint[1], bbox_hw), img_size[0]-1-bbox_hw)
