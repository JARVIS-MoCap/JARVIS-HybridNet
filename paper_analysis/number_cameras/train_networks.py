...
for i,set in enumerate(sets):
    ...
    train.train_efficienttrack('CenterDetect', 'Hand_Num_Cameras',
                50, 'ecoset', run_name = f'Camera_Numbers_{num_cameras}_{i}', cameras_to_use = camera_list)

    train.train_efficienttrack('KeypointDetect', 'Hand_Num_Cameras',
                100, 'ecoset', run_name = f'Camera_Numbers_{num_cameras}_{i}', cameras_to_use = camera_list)

    train.train_hybridnet('Hand_Num_Cameras', 50,
                'latest', None, '3D_only',
                run_name = f'Camera_Numbers_{num_cameras}_{i}', cameras_to_use = camera_list)

    train.train_hybridnet('Hand_Num_Cameras', 50,
                None, 'latest', 'all', finetune = True,
                run_name = f'Camera_Numbers_{num_cameras}_{i}_finetune', cameras_to_use = camera_list)
