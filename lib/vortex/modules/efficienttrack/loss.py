...
class LossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.heatmaps_loss_factor = [1.0,1.0]
...
class MultiLossFactory(nn.Module):
    def __init__(self, cfg):
        ...
        self.num_joints = cfg.EFFICIENTTRACK.NUM_JOINTS
        self.num_stages = 2
        ...
        self.heatmaps_loss = \
            nn.ModuleList(
                [
                    ...
                    for with_heatmaps_loss in [True, True]
                ]
            )
        self.heatmaps_loss_factor = [1.0,1.0]
        ...
        self.ae_loss = \
            nn.ModuleList(
                [
                    AELoss(cfg.HRNET.LOSS.AE_LOSS_TYPE) if with_ae_loss else None
                    for with_ae_loss in [False, False]
                ]
            )
        self.push_loss_factor = [0,0]
        self.pull_loss_factor = [0,0]
    ...
    def _init_check(self, cfg):
        assert isinstance(cfg.HRNET.LOSS.WITH_HEATMAPS_LOSS, (list, tuple)), \
            'LOSS.WITH_HEATMAPS_LOSS should be a list or tuple'
        assert isinstance(cfg.HRNET.LOSS.HEATMAPS_LOSS_FACTOR, (list, tuple)), \
            'LOSS.HEATMAPS_LOSS_FACTOR should be a list or tuple'
        assert isinstance(cfg.HRNET.LOSS.WITH_AE_LOSS, (list, tuple)), \
            'LOSS.WITH_AE_LOSS should be a list or tuple'
        assert isinstance(cfg.HRNET.LOSS.PUSH_LOSS_FACTOR, (list, tuple)), \
            'LOSS.PUSH_LOSS_FACTOR should be a list or tuple'
        assert isinstance(cfg.HRNET.LOSS.PUSH_LOSS_FACTOR, (list, tuple)), \
            'LOSS.PUSH_LOSS_FACTOR should be a list or tuple'
        assert len(cfg.HRNET.LOSS.WITH_HEATMAPS_LOSS) == cfg.HRNET.LOSS.NUM_STAGES, \
            'LOSS.WITH_HEATMAPS_LOSS and LOSS.NUM_STAGE should have same length, got {} vs {}.'.\
                format(len(cfg.HRNET.LOSS.WITH_HEATMAPS_LOSS), cfg.HRNET.LOSS.NUM_STAGES)
        assert len(cfg.HRNET.LOSS.WITH_HEATMAPS_LOSS) == len(cfg.HRNET.LOSS.HEATMAPS_LOSS_FACTOR), \
            'LOSS.WITH_HEATMAPS_LOSS and LOSS.HEATMAPS_LOSS_FACTOR should have same length, got {} vs {}.'.\
                format(len(cfg.HRNET.LOSS.WITH_HEATMAPS_LOSS), len(cfg.HRNET.LOSS.HEATMAPS_LOSS_FACTOR))
        assert len(cfg.HRNET.LOSS.WITH_AE_LOSS) == cfg.HRNET.LOSS.NUM_STAGES, \
            'LOSS.WITH_AE_LOSS and LOSS.NUM_STAGE should have same length, got {} vs {}.'.\
                format(len(cfg.HRNET.LOSS.WITH_AE_LOSS), cfg.HRNET.LOSS.NUM_STAGES)
        assert len(cfg.HRNET.LOSS.WITH_AE_LOSS) == len(cfg.HRNET.LOSS.PUSH_LOSS_FACTOR), \
            'LOSS.WITH_AE_LOSS and LOSS.PUSH_LOSS_FACTOR should have same length, got {} vs {}.'. \
                format(len(cfg.HRNET.LOSS.WITH_AE_LOSS), len(cfg.HRNET.LOSS.PUSH_LOSS_FACTOR))
        assert len(cfg.HRNET.LOSS.WITH_AE_LOSS) == len(cfg.HRNET.LOSS.PULL_LOSS_FACTOR), \
            'LOSS.WITH_AE_LOSS and LOSS.PULL_LOSS_FACTOR should have same length, got {} vs {}.'. \
                format(len(cfg.HRNET.LOSS.WITH_AE_LOSS), len(cfg.HRNET.LOSS.PULL_LOSS_FACTOR))
